...
class TapHoldManager {
	...
	class KeyManager {
		...
		DeclareHotkeys(){
			if (this.window)
				HotIfWinNotActive  this.window ; sets the hotkey window context if window option is passed-in
			...
			if (this.window)
				HotIfWinNotActive  ; restores hotkey window context to default
		}
		...
		KeyEvent(state, *){
			...
			if (state){
				; Key went down
				this.sequence++
				this.FireCallback.Call(this, this.sequence, 2)
